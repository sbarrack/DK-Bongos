...
#define press(num, state) Joystick.button(num, state)
#define stick(x, y) Joystick.X(x); Joystick.Y(y)
#define stickC(x, y) Joystick.Zrotate(x); Joystick.Z(y)

#define USB_MAX	0x3FF
#define USB_MID	0x200
#define USB_MIN	0
enum Button { x = 1, b, a, y, zl, zr, l, r, sel, start };
...
inline void toUSB() {
	...
	press(y, n64data.cd);
	press(zl, n64data.cl);
	press(sel, n64data.cr);
	// must safely convert s8 to u16
	stick(((u16) (((~((u8) n64data.sx)) & 0x80) 
		| (((u8) n64data.sx) & 0x7F))) << 2,
		((u16) (((~((u8) n64data.sy)) & 0x80) 
		| (((u8) n64data.sy) & 0x7F))) << 2);
	Joystick.send_now();
}
